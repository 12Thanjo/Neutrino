...
var Compiler = function(input, const_dict, config){
	...
	var legacy_error = function(message, position){
		if(config.legacy){
			error("Legacy mode error: " + message + ". This limitation may change in the future.", position);
		}
	}
	...
	evaluators.set("iterate", (expr)=>{
		...
		if(!config.legacy){	
			...
			output += 'for(var '+expr.value + ' in ' + id + "){var " + expr.value  + "=" + id + "[" + expr.key + "];" + n;
		}
	...
	evaluators.set('species', (expr)=>{
		...
		if(!config.legacy){
			...
			output += t() + `${id}.$map={};${id}.get=function(id){return ${id}.$map[id];};${id}.has=function(id){return ${id}.$map[id]!=null;};${id}.forEach=function(cb){for(var key in $map){cb(key)};};${id}.delete=function(id){delete ${id}.$map[id];};` + comment(expr.position);
		}
	...
	evaluators.set('class', (expr)=>{
		...
		if(!config.legacy){
			...
			output += t() + `${id}.$i=0;${id}.resetI=function(){${id}.$i=0;};${id}.$map={};${id}.get=function(id){return ${id}.$map[id];};${id}.has=function(id){return ${id}.$map[id]!=null;};${id}.forEach=function(cb){for(var key in $map){cb(key)};};${id}.delete=function(id){delete ${id}.$map[id];};` + comment(expr.position);
		}
	...
	evaluators.set('access', (expr)=>{
		legacy_error("access properties are not supported", expr.position);
		var output = "";
	...
	evaluators.set('error', (expr)=>{
		...
		if(!config.node){
			...
			output += t() + "throw new " + expr.error + "(" + no_tab_evaluate(expr.message) + ");" + n;
			tab_depth -= 1;
			output += t() + "}catch(e){console.log('\x1b[31m'+e.stack+'\x1b[37m');process.exit();};" + comment(expr.position, expr.position);
		}
	...
	evaluators.set('import', (expr)=>{
		legacy_error("imports are not supported", expr.position);
