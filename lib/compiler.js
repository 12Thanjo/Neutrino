...
var Compiler = function(input, const_dict, config){
	...
	config.compatibility = 	config.compatibility ?? false;
	...
	var compatibility_error = function(message, position){
		error("Compatibility error: " + message + ". This limitation may change in the future.", position);
	}
	...
	evaluators.set('species', (expr)=>{
		compatibility_error("species not supported. Use struct instead");
		var output = "";
		var id = no_tab_evaluate(expr.id);

		output += t() + `let ${id}=function${parse_params(expr.params)}{` + n;
		tab_depth += 1;
		output += t() + "let $this=this;let private={};this.$op={};" + id + ".$map.set(" + expr.params[0].value + ",this);" + n;
		output += evaluate(expr.program);
		tab_depth -= 1;
		output += t() + "};" + comment(expr.position);
	...
	evaluators.set('class', (expr)=>{
		compatibility_error("class not supported. Use struct instead");
		var output = "";
		var id = no_tab_evaluate(expr.id);

		output += t() + `let ${id}=function${parse_params(expr.params)}{` + n;
		tab_depth += 1;
		output += t() + "let $this=this;let private={};this.$op={};" + id + ".$map.set(" + id + ".$i,this);let id=" + id + ".$i;" + id + ".$i+=1;" + n;
		output += evaluate(expr.program);
		tab_depth -= 1;
		output += t() + "};" + comment(expr.position);
	...
	evaluators.set('import', (expr)=>{
		compatibility_error("imports are not supported");
